...
const CORE_MODULES = ['Évaluation', 'Présence', 'Journal', 'Comptabilité', 'RH & paie', 'Cantine', 'Moments', 'Comportement', 'Documents', 'Accidents', 'Inscriptions', 'Recrutement', 'Budget', 'Inventaire']
const TABS = [
...
function Toggle({ on, onClick, label, desc }) {
  return (
    <button type="button" onClick={onClick} aria-pressed={on}
      className="flex items-center gap-3 w-full text-left p-3 rounded-xl border border-line hover:bg-canvas transition">
...
export default function Settings() {
  ...
  const save = () => {
    if (!f.schoolName.trim()) return toast.error('Le nom de l’école est requis.')
    if (!f.levels.length) return toast.error('Une école accueille au moins un niveau.')
    ...
    // devise appliquée en mémoire (money() la lit) + langue par défaut de l'école
    setCurrency(f.currency)
    saveSettings({ ...f })
    // modules : dans leur propre clé, lue à l'import par la navigation
    ...
    if (levelsChanged || modsChanged) {
      toast.success('Paramètres enregistrés — la navigation se recharge pour appliquer les modules et cycles.')
      setTimeout(() => location.reload(), 750)
    } else {
      toast.success('Paramètres enregistrés — appliqués partout.')
    }
  }
  ...
  return (<>
    ...
    <div className="flex gap-1 overflow-x-auto scroll-thin -mx-1 px-1 mb-5">
      {TABS.map(t => {
        const Ic = t.icon; const on = tab === t.k
        return (
    ...
    <div className="grid lg:grid-cols-[1fr_320px] gap-5 items-start">
      <div className="space-y-5">
        ...
        {tab === 'etab' && (
          <Card className="p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <Field label="Directeur / directrice"><Input value={f.director} onChange={e => set('director', e.target.value)} /></Field>
            ...
            <div className="text-xs font-bold uppercase tracking-wide accent-text mt-5 mb-2">Coordonnées</div>
            <div className="grid sm:grid-cols-2 gap-3">
        ...
        {tab === 'cycles' && (
          <Card className="p-6">
            <h3 className="font-bold flex items-center gap-2 mb-1"><Layers size={18} className="accent-text" /> Cycles &amp; niveaux accueillis</h3>
            <p className="text-sm text-muted mb-4">Cochez les niveaux que votre école accueille. Les modules et écrans s’adaptent : une crèche ne verra jamais les écrans du primaire.</p>
            {[['Petite enfance', EARLY_YEARS], ['Primaire', PRIMARY]].map(([grp, keys]) => (
            ...
            <p className="text-xs text-muted mt-1">{f.levels.length} niveau(x) actif(s). L’enregistrement recharge la navigation.</p>
          </Card>
        )}
        ...
        {tab === 'modules' && (
          <>
            <Card className="p-6">
              <h3 className="font-bold flex items-center gap-2 mb-1"><Boxes size={18} className="accent-text" /> Modules optionnels</h3>
              <p className="text-sm text-muted mb-4">Activez ce dont votre école a besoin. Ces modules existent, testés — vous les allumez d’un clic.</p>
              <div className="space-y-2">
                {OPTIONAL_MODULES.map(m => (
                  <Toggle key={m} on={mods[m]} onClick={() => toggleMod(m)} label={MOD_META[m].label} desc={MOD_META[m].desc} />
                ))}
              </div>
            </Card>
            <Card className="p-6">
              <div className="text-xs font-bold uppercase tracking-wide accent-text mb-2 flex items-center gap-2"><ShieldCheck size={14} /> Le cœur — toujours actif</div>
              <p className="text-sm text-muted mb-3">Ce qui fait vivre l’école tous les jours ne se désactive pas.</p>
              <div className="flex flex-wrap gap-1.5">
                {CORE_MODULES.map(c => <span key={c} className="text-xs font-semibold px-2.5 py-1 rounded-full accent-soft accent-text">{c}</span>)}
              </div>
            </Card>
          </>
        )}
        ...
        {tab === 'local' && (
          <Card className="p-6">
            <h3 className="font-bold flex items-center gap-2 mb-4"><Globe size={18} className="accent-text" /> Localisation &amp; finances</h3>
            <div className="grid sm:grid-cols-2 gap-3">
              <Field label="Devise" hint="Utilisée pour tous les montants (frais, paie, budget).">
              ...
              <Field label="Langue par défaut" hint="La langue d’un nouvel appareil. Chacun peut la changer.">
                <Select value={f.locale || 'fr'} onChange={e => set('locale', e.target.value)}>
                  <option value="fr">Français</option>
                  <option value="ar">العربية (arabe · droite-à-gauche)</option>
                </Select>
              </Field>
            </div>
            <div className="mt-4 p-3 rounded-xl bg-canvas text-sm text-muted">
              Exemple de montant : <b className="text-ink">{money(2400)}</b> · <b className="text-ink">{money(150)}</b>
            </div>
          </Card>
        )}
        ...
        {tab === 'brand' && (
          <Card className="p-6">
            <h3 className="font-bold flex items-center gap-2 mb-1"><Palette size={18} className="accent-text" /> Marque &amp; couleur</h3>
            <p className="text-sm text-muted mb-4">Chaque teinte est validée pour l’accessibilité. Utilisée sur la page publique et les documents.</p>
        ...
        {tab === 'data' && (
          <>
            <Card className="p-6">
              <h3 className="font-bold flex items-center gap-2 mb-1"><Database size={18} className="accent-text" /> Vos données vous appartiennent</h3>
              <p className="text-sm text-muted mb-4">Exportez l’intégralité de votre école au standard international OneRoster v1.2 — un clic, aucun verrou. Vous pouvez partir quand vous voulez.</p>
              <Link to="/app/interop"><Btn variant="soft"><Download size={16} /> Exporter (OneRoster v1.2)</Btn></Link>
            </Card>
            {isDemo && (
              <Card className="p-6" style={{ borderColor: 'var(--line)' }}>
                <div className="text-xs font-bold uppercase tracking-wide mb-2 flex items-center gap-2" style={{ color: 'var(--coral, #B93E46)' }}><AlertTriangle size={14} /> Données de démonstration</div>
                <p className="text-sm text-muted mb-3">Cette base est la démo (École Al-Nour). Vous pouvez la remettre à zéro pour repartir d’une école propre.</p>
                <Btn variant="danger" onClick={() => setResetOpen(true)}>Réinitialiser la démonstration</Btn>
              </Card>
            )}
          </>
        )}
      ...
      <div className="lg:sticky lg:top-20">
        ...
        <Card className="p-4">
          ...
          <div className="mt-4 space-y-1.5 text-sm">
            ...
            <div className="flex justify-between"><span className="text-muted">Devise</span><span className="font-medium">{f.currency || 'DT'}</span></div>
            <div className="flex justify-between"><span className="text-muted">Langue</span><span className="font-medium">{f.locale === 'ar' ? 'العربية' : 'Français'}</span></div>
            <div className="flex justify-between"><span className="text-muted">Niveaux</span><span className="font-medium">{f.levels.length}</span></div>
            <div className="flex justify-between"><span className="text-muted">Modules optionnels</span><span className="font-medium">{activeMods} / {OPTIONAL_MODULES.length}</span></div>
          </div>
        </Card>
      </div>
    </div>

    <Modal open={resetOpen} onClose={() => setResetOpen(false)} title="Réinitialiser la démonstration ?"
      footer={<><Btn variant="ghost" onClick={() => setResetOpen(false)}>Annuler</Btn><Btn variant="danger" onClick={doReset}>Oui, réinitialiser</Btn></>}>
      <p className="text-sm text-muted">Toutes les données locales de démonstration seront effacées et l’école Al-Nour sera régénérée. Sans effet sur une vraie école.</p>
    </Modal>
  </>)
}
