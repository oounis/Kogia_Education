...
export default function Login(){
  ...
  const go=()=>{ const u=login(email,pw); if(u&&!u.disabled){nav('/app')} else setErr(u&&u.disabled?t('Ce compte a été désactivé. Contactez la direction.'):t('E-mail ou mot de passe incorrect.')) }
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="hidden lg:flex flex-col justify-between p-12 text-white" style={{background:'linear-gradient(150deg,#7539E4,#8B5CF6)'}}>
        ...
        <div><div className="inline-flex items-center gap-2 text-xs font-semibold px-3 py-1 rounded-full bg-white/15 mb-4"><Zap size={13}/> {t('Pas un ERP scolaire de plus')}</div>
          <h1 className="text-4xl font-extrabold leading-tight max-w-[16ch]">{t("L'école qu'on a envie d'ouvrir.")}</h1>
      ...
      <div className="flex items-center justify-center p-8 bg-canvas">
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} className="w-full max-w-sm">
          ...
          <div className="flex items-center justify-between"><h2 className="text-2xl font-extrabold">{t('Bon retour')}</h2><LangSwitch/></div>
          <p className="text-muted text-sm mb-6">{t('Connectez-vous à votre portail.')}</p>
          <div className="space-y-3">
            <Input value={email} onChange={e=>{setEmail(e.target.value);setErr(false)}} placeholder={t('E-mail')}/>
            <Input type="password" value={pw} onChange={e=>{setPw(e.target.value);setErr(false)}} onKeyDown={e=>e.key==='Enter'&&go()} placeholder={t('Mot de passe')}/>
            ...
            <Btn size="lg" className="w-full" onClick={go} style={{background:BRAND.indigo}}>{t('Se connecter')}</Btn>
          </div>
          <div className="mt-6"><div className="text-xs text-muted mb-2">{t('Démo — connexion en un clic :')}</div>
            <div className="flex flex-wrap gap-2">
              <button onClick={()=>{loginAs('u_owner');nav('/app')}} className="text-xs font-semibold px-3 py-1.5 rounded-full border border-line bg-white" style={{color:ROLE.owner.color}}>Kogia Group ({t(ROLE.owner.label)})</button>
              {quick.map(u=>{const r=ROLE[u.role];return <button key={u.id} onClick={()=>{loginAs(u.id);nav('/app')}} className="text-xs font-semibold px-3 py-1.5 rounded-full border border-line bg-white" style={{color:r.color}}>{t(r.label)}</button>})}
            </div>
