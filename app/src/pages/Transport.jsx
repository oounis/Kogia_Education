...
export default function Transport(){
  const d=db()
  return (<>
    <PageHead title="Transport" sub={`${d.routes.length} circuits`}/>
    <div className="grid md:grid-cols-2 gap-4">
      {d.routes.map(r=>(<Card key={r.id} className="p-5">
        <div className="flex items-center gap-3 mb-3"><span className="w-11 h-11 rounded-xl grid place-items-center text-white" style={{background:'#FFA62B'}}><Bus size={20}/></span>
          <div><div className="font-bold">{r.name}</div><div className="text-sm text-muted">Bus {r.bus} · {r.students} élèves</div></div></div>
        <div className="text-sm text-muted flex items-center gap-1 mb-2"><User size={14}/> Chauffeur : <b className="text-ink">{r.driver}</b></div>
