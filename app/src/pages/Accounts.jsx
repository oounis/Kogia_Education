...
import { roleAvatar, avatarBg, resolveUserAvatar } from '../people.js'
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {us.map(u=>(
              <button key={u.id} onClick={()=>setView(u)} className={`card p-4 flex items-center gap-3 text-left hover:shadow-lg hover:-translate-y-0.5 transition w-full ${u.disabled?'opacity-60':''}`}>
                <span className="w-11 h-11 rounded-full overflow-hidden grid place-items-center shrink-0" style={{background:avatarBg(u.id),opacity:u.disabled?.5:1}}><img src={resolveUserAvatar(u)} alt="" className="w-full h-full object-contain"/></span>
