...
const inLabel = n => n === 0 ? "aujourd'hui" : n === 1 ? 'demain' : `dans ${n} j`
  ...
  return (
    <Menu as="div" className="relative hidden md:block">
      <Menu.Button className="flex items-center gap-2 pl-2.5 pr-3 py-1.5 rounded-2xl hover:shadow transition text-left"
        title="Fêtes et journées à venir"
        ...
        <div className="leading-none min-w-0">
          <div className="font-extrabold text-[13px] accent-text truncate max-w-[180px]">
            {head.kind === 'prochain' ? head.label : head.label}
          </div>
          <div className="text-[11px] font-semibold text-muted mt-0.5">
            {head.kind === 'ferie' ? "jour férié · aujourd'hui"
              : head.kind === 'journee' ? "aujourd'hui"
              : `prochain congé · ${inLabel(head.inDays)}`}
          </div>
        </div>
      ...
      <Menu.Items className="absolute right-0 mt-2 w-72 card p-0 shadow-2xl z-50 overflow-hidden focus:outline-none">
        <div className="p-5 text-center"
          ...
          <div className="text-sm font-extrabold mt-1">{head.label}</div>
          <div className="text-xs text-muted mt-0.5 capitalize">
            {head.kind === 'prochain'
              ? `${format(new Date(head.d), 'EEEE d MMMM', { locale: fr })} · ${inLabel(head.inDays)}`
              : format(appNow(), 'EEEE d MMMM yyyy', { locale: fr })}
          </div>
        </div>
        <div className="p-3">
          <div className="text-[11px] font-bold uppercase tracking-wide text-muted px-2 mb-1">À venir</div>
          {agenda.map(x => (
            <div key={x.kind + x.d + x.label} className="flex items-center gap-2.5 px-2 py-1.5 rounded-xl hover:bg-canvas">
              ...
              <span className="min-w-0 flex-1">
                <span className="block text-[13px] font-semibold truncate">{x.label}</span>
                <span className="block text-[11px] text-muted capitalize">
                  {format(new Date(x.d), 'EEE d MMM', { locale: fr })} · {inLabel(x.inDays)}{x.lune ? ' · selon la lune' : ''}
                </span></span>
              {x.kind === 'ferie' && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full accent-soft accent-text shrink-0">FÉRIÉ</span>}
            </div>))}
        </div>
      </Menu.Items>
    </Menu>
